...
static void *CaseFilterCreateServerConfig(apr_pool_t *p,server_rec *s)
    {
    CaseFilterConfig *pConfig=(CaseFilterConfig*)apr_pcalloc(p,sizeof *pConfig);
...
static void CaseFilterInsertFilter(request_rec *r)
    {
    CaseFilterConfig *pConfig=(CaseFilterConfig*)ap_get_module_config(r->server->module_config,
                                                   &case_filter_module);
